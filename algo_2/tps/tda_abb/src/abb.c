...
abb_insertar(abb_t* arbol, void* elemento)
{
    ...
    arbol->nodo_raiz = abb_insertar_recursivo_aux(arbol->nodo_raiz, elemento,
                                                  arbol->comparador);
...
abb_quitar(abb_t* arbol, void* elemento)
{
    ...
    arbol->nodo_raiz = abb_quitar_recursivo_aux(
        arbol->nodo_raiz, elemento, &elemento_quitado, arbol->comparador);
...
abb_buscar(abb_t* arbol, void* elemento)
{
    ...
    return abb_buscar_recursivo_aux(arbol->nodo_raiz, elemento,
                                    arbol->comparador);
}
...
size_t
abb_con_cada_elemento(abb_t* arbol, abb_recorrido recorrido,
...
size_t
abb_recorrer(abb_t* arbol, abb_recorrido recorrido, void** array,
             size_t tamanio_array)
{
    ...
    if (recorrido == PREORDEN)
        return abb_recorrer_preorden_aux(arbol->nodo_raiz, array,
                                         tamanio_array);
    ...
    if (recorrido == POSTORDEN)
        return abb_recorrer_postorden_aux(arbol->nodo_raiz, array,
                                          tamanio_array);
...
nodo_abb_t*
abb_insertar_recursivo_aux(nodo_abb_t* raiz, void* elemento,
                           abb_comparador comparador)
{
    ...
    if (comparacion <= 0)
        raiz->izquierda =
            abb_insertar_recursivo_aux(raiz->izquierda, elemento, comparador);
    else
        raiz->derecha =
            abb_insertar_recursivo_aux(raiz->derecha, elemento, comparador);

    return raiz;
}

nodo_abb_t*
abb_quitar_recursivo_aux(nodo_abb_t* raiz, void* elemento,
                         void** elemento_quitado, abb_comparador comparador)
{
    ...
    if (comparacion == 0) {
        ...
        raiz->izquierda = abb_quitar_recursivo_aux(
            raiz->izquierda, elemento, elemento_quitado, comparador);
    }
    else {
        raiz->derecha = abb_quitar_recursivo_aux(raiz->derecha, elemento,
...
void*
abb_buscar_recursivo_aux(nodo_abb_t* raiz, void* elemento,
                         abb_comparador comparador)
{
    if (!raiz || !comparador)
        return NULL;
...
abb_recorrer_preorden_aux(nodo_abb_t* raiz, void** array, size_t tamanio_array)
{
    ...
    cantidad += abb_recorrer_preorden_aux(raiz->izquierda, array + cantidad,
                                          tamanio_array - cantidad);
    cantidad += abb_recorrer_preorden_aux(raiz->derecha, array + cantidad,
                                          tamanio_array - cantidad);
...
abb_recorrer_inorden_aux(nodo_abb_t* raiz, void** array, size_t tamanio_array)
{
    ...
    size_t cantidad = 0;
    cantidad += abb_recorrer_inorden_aux(raiz->izquierda, array + cantidad,
                                         tamanio_array);
    array[cantidad++] = raiz->elemento;
    cantidad += abb_recorrer_inorden_aux(raiz->derecha, array + cantidad,
                                         tamanio_array);
...
abb_recorrer_postorden_aux(nodo_abb_t* raiz, void** array, size_t tamanio_array)
{
    ...
    size_t cantidad = 0;
    cantidad += abb_recorrer_postorden_aux(raiz->izquierda, array + cantidad,
                                           tamanio_array);
    cantidad += abb_recorrer_postorden_aux(raiz->derecha, array + cantidad,
                                           tamanio_array);
